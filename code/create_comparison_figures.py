...
def load_model(model_path: str) -> Any:
    ...
    try:
        if model_path.endswith('.json'):
            ...
            print(f"⚠️ 지원하지 않는 모델 형식: {model_path}")
            return None
        return model
    except ImportError as e:
        print(f"⚠️ 모델 로드 실패 (모듈 미설치): {os.path.basename(model_path)}")
        return None
...
def find_models(models_dir: str) -> Dict[str, str]:
    ...
    patterns = [
        ...
        ('xgboost', 'xgboost_best_model.json'),
        ('catboost', 'catboost_best_model.cbm'),
...
def plot_roc_comparison(
    ...
    if save_path:
        ...
        print(f"✅ ROC Comparison 저장: {save_path}")
...
def plot_pr_comparison(
    ...
    if save_path:
        fig.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"✅ PR Comparison 저장: {save_path}")
...
def plot_calibration_comparison(
    ...
    if save_path:
        fig.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"✅ Calibration Comparison 저장: {save_path}")
...
def plot_combined_comparison(
    ...
    if save_path:
        fig.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"✅ Combined Comparison 저장: {save_path}")
...
def plot_shap_comparison(
    ...
    if save_path:
        fig.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"✅ SHAP Comparison 저장: {save_path}")
...
def create_comparison_figures(
    ...
    # 고해상도 버전 (TIFF)
    fig = plot_combined_comparison(roc_data, pr_data, cal_data, baseline=baseline)
    fig.savefig(os.path.join(output_dir, 'comparison_combined.tiff'), 
                dpi=600, bbox_inches='tight', format='tiff')
    print(f"✅ 고해상도 TIFF 저장: {os.path.join(output_dir, 'comparison_combined.tiff')}")
    plt.close(fig)
    
    # PDF 버전
    fig = plot_combined_comparison(roc_data, pr_data, cal_data, baseline=baseline)
    fig.savefig(os.path.join(output_dir, 'comparison_combined.pdf'),
                bbox_inches='tight', format='pdf')
    print(f"✅ PDF 저장: {os.path.join(output_dir, 'comparison_combined.pdf')}")
    plt.close(fig)
    ...
    if HAS_SHAP:
        ...
        for model_name, model in loaded_models.items():
            ...
            try:
                result = compute_shap_values(model, X_test, feature_names, y=y_test, max_samples=2000)
        ...
        if len(shap_data) > 0:
            ...
            # 고해상도 TIFF
            fig = plot_shap_comparison(shap_data)
            if fig:
                fig.savefig(os.path.join(output_dir, 'comparison_shap.tiff'),
                            dpi=600, bbox_inches='tight', format='tiff')
                print(f"✅ SHAP TIFF 저장: {os.path.join(output_dir, 'comparison_shap.tiff')}")
                plt.close(fig)
                
                # PDF
                fig = plot_shap_comparison(shap_data)
                fig.savefig(os.path.join(output_dir, 'comparison_shap.pdf'),
                            bbox_inches='tight', format='pdf')
                print(f"✅ SHAP PDF 저장: {os.path.join(output_dir, 'comparison_shap.pdf')}")
                plt.close(fig)
    else:
